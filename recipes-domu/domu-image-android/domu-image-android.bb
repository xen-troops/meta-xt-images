SUMMARY = "Base for build of Android"
...
#
# Some details:
# 'depth=1' reduces amount of data downloaded from server and is most significant
#           reducer of fetch time
...
# If XT_ANDROID_PREBUILDS_DIR is not set, we need to use proprietary sources for build of graphics
python __anonymous () {
    if not d.getVar("XT_ANDROID_PREBUILDS_DIR", expand=True):
        d.appendVar("SRC_URI", ";groups=all")
}
...
# These two are not suppoesed to be changed.
